...
$(document).ready(function() {
    $('#typeahead-input').keyup(() => {
        ...
        if(input.value.length > 0) {
            $.get('/stocksearch?stock=' + input.value, function(data) {
                for(var i = 0; i < data.length; i++) {
                    var company = {};
                    company.name = data[i].Symbol
                    company.marketCap = format({prefix: '$', integerSeparator: ','})(parseInt((data[i].MarketCap)/1000)*1000);
                    company.companyName = data[i].Name
                    names.push(company);
                }
